    ...
    return function<T extends new (...args: any[]) => MachineBlock>(target: T): T {
        return class extends target {
            public constructor(...args: any[]) {
                super(...args);
                LiquidMachine.Provider.setPipesConnecting(this.id);
                const tilePrototype = TileEntity.getPrototype(this.id) as LiquidMachine.TileEntity; 
                
                if(tilePrototype == null) {
                    throw new ReferenceError("LiquidMachine does not can don't contain tile entity prototype");
                }
                const policies: Record<string, LiquidMachine.SlotPolicy> = {};
                const getSlots = new Set<string>();
                const addSlots = new Set<string>();
                const elements = tilePrototype.getScreenByName().getContent().elements;
                ...
                for(const descriptor of descriptors) {
                    ...
                    if(descriptor.action == "add") {
                        ...
                        transportLiquid.add(descriptor.liquidName);
                    }

                    if(receiveLiquid.size > 0) {
                        data.canReceiveLiquid = (liquidName, side) => {
                            return receiveLiquid.has(liquidName); //side require can appear in future
                        }
                    }
                    if(transportLiquid.size > 0) {
                        data.canTransportLiquid = (liquidName, side, ) => {
                            return transportLiquid.has(liquidName);
                        }
                    }
                }
                LiquidMachine.Provider.injectInit(tilePrototype, descriptors, policies);
                LiquidMachine.Provider.injectTick(tilePrototype, descriptors);
            }
        }
    }
}

namespace LiquidMachine {
    ...
    export namespace Provider {
        export function setPipesConnecting(id: number): void {
            ICRender.getGroup("galacticraft.machine_liquid_connecting_0").add(id, 0);
            ICRender.getGroup("galacticraft.machine_liquid_connecting_1").add(id, 1);
            ICRender.getGroup("galacticraft.machine_liquid_connecting_2").add(id, 2);
            ICRender.getGroup("galacticraft.machine_liquid_connecting_3").add(id, 3);
        }

        export function addLiquidItemPolicy(descriptor: LiquidMachine.LiquidDescriptor, policies: Record<string, LiquidMachine.SlotPolicy>): void {
            let policy: LiquidMachine.SlotPolicy = null;

            if(descriptor.action == "add") {
                policy = (tileEntity, item) => completeLiquidCheck(tileEntity, descriptor, this.isValidNotEmpty(item, descriptor.liquidName));
            } else {
                policy = (tileEntity, item) => {
                    return this.completeLiquidCheck(tileEntity, descriptor, isValidEmpty(item, descriptor.liquidName));
                };
            }
            policies[descriptor.slotName] = policy;
        }

        export function isValidEmpty({ id, data, extra }: ItemInstance, liquidName: string, canCanisterHavePartial: boolean = true): boolean {
        ...
        export function isValidNotEmpty({ id, data, extra }: ItemInstance, liquidName: string): boolean {
        ...
        export function completeLiquidCheck(tileEntity: LiquidMachine.TileEntity, descriptor: LiquidMachine.LiquidDescriptor, success: boolean): boolean {
            if(success == true) {
        ...
        export function addLiquid(tileEntity: LiquidMachine.TileEntity, descriptor: LiquidMachine.LiquidDescriptor): boolean {
        ...
        export function takeLiquid(tileEntity: LiquidMachine.TileEntity, descriptor: LiquidMachine.LiquidDescriptor): boolean {
            const slot = tileEntity.container.getSlot(descriptor.slotName);
            const amount = tileEntity.liquidStorage.getAmount(descriptor.liquidName);
        ...
        export function injectInit(tilePrototype: LiquidMachine.TileEntity, descriptors: LiquidMachine.LiquidDescriptor[], policies: Record<string, LiquidMachine.SlotPolicy>): void {
            const lastInit = tilePrototype.init;

            tilePrototype.init = function(this: LiquidMachine.TileEntity) {
                ...
                for(const descriptor of descriptors) {
                    ...
                    if(
                        descriptor.action == "add" && isValidNotEmpty(slot, descriptor.liquidName) ||
                        descriptor.action == "get" && isValidEmpty(slot, descriptor.liquidName)
                    ) {
                ...
                for(const slotName in policies) {
                    ...
                    this.container.setSlotAddTransferPolicy(slotName, (container, str, id, count, data, extra, playerUid) => {
        ...
        export function injectTick(tilePrototype: LiquidMachine.TileEntity, descriptors: LiquidMachine.LiquidDescriptor[]) {
                ...
                if(this.liquidSlotChecks?.size > 0) {
                    this.liquidSlotChecks.forEach((descriptor, key) => {
                        ...
                        if((this.data.energy || 0) >= descriptor.requireEnergy) {
                            ...
                            if(
                                descriptor.action == "add" && (!isValidNotEmpty(slot, descriptor.liquidName) || addLiquid(this, descriptor)) ||
                                descriptor.action == "get" && (!isValidEmpty(slot, descriptor.liquidName) || takeLiquid(this, descriptor))
                            ) {
