class Canister extends GItem {
  ...
  public static input(
    ...
    const getInput = (id, value) => {
      return (
        ...
        data[canister] != 40 &&
        Storage.get(container, slot, id, value)
      );
    };

    if (
      getInput("id", Canister.get(canister)) &&
    ...
    } else if (bucket && getInput("id", ItemID["bucket_of_" + canister])) {
      if (data.energy) data.energy -= 45;
      return Storage.set(slot, 325, container, 1, 0), (data[canister] += 5);
    }
  }
  ...
  public static output(
    ...
    if (World.getThreadTime() % 20 == 0 && data[canister] > 0) {
      if (
