...
Callback.addCallback("ServerPlayerTick", (playerUid, isPlayerDead) => {
    ...
    const noOxygen = TagRegistry.getDimensionTags(Entity.getDimension(playerUid)).includes("no_oxygen");
    if(!noOxygen) return;
    ...
    if(World.getThreadTime() % 20 === 0) {
        ...
        if(container != null) {
            ...
            if(amount <= 0) {
                ...
                Game.message(Native.Color.RED + Translation.translate("message.galacticraft.no_oxygen_danger"))
            };
        };
    };
});
...
Callback.addCallback("NativeGuiChanged", (screenName) => {
    if(screenName == EScreenName.INVENTORY_SCREEN || screenName === EScreenName.INVENTORY_SCREEN_POCKET || screenName === EScreenName.CREATIVE_INVENTORY_SCREEN || screenName === EScreenName.SURVIVAL_INVENTORY_SCREEN) {
