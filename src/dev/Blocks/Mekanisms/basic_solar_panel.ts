...
class SolarPanel extends Generator {
    ...
    onTick(): void {
        ...
        if (this.blockSource.getLightLevel(
            this.x,
            this.y + 1, this.z) >= 2) {
            this.container.setText(
                "LightLevel",
        ...
        if (
            World.getThreadTime() % 60 == 0 && this.blockSource.getLightLevel(
                this.x,
                this.y + 1,
                this.z) <= 1 &&
            this.data.energy <= 1) {
...
SpacesMachine.registerStandartMachine(BlockID.basic_solar_panel, new SolarPanel(SolarGeneratorUI));
